// webserver.ino - webserver cho STU-control
...
const char HTTP_HEAD[] PROGMEM =
  ...
  "var cn,x,lt;"
  "cn=120;"
  "x=null;"                  // Cho phép hủy cấu hình lại thiết bị nếu trong 120 giây không tương tác
  "function u(){"
    "if(cn>=0){"
      "document.getElementById('t').innerHTML='Khởi động lại thiết bị trong  '+cn+' seconds';"
      "cn--;"
      "setTimeout(u,1000);"
    "}"
  "}"
  ...
  "<div style='text-align:center;'><h3>{ha} Module</h3><h2>{h}</h2></div>"; //end http head.
const char HTTP_SCRIPT_CONSOL[] PROGMEM =
  ...
  "function l(p){"               // Console log and command service
    ...
    "return false;"
  "}"
  "</script>";  //end http script console
...
const char HTTP_BTN_MENU2[] PROGMEM =
 // "<br/><form action='/md' method='post'><button>Configure Module</button></form>" //httu edited.
  "<br/><form action='/w0' method='post'><button>CẤU HÌNH WiFi</button></form>";
const char HTTP_BTN_MENU3[] PROGMEM =
  "<br/><form action='/mq' method='post'><button>CẤU HÌNH MQTT</button></form>"
#ifdef USE_DOMOTICZ
  "<br/><form action='/dm' method='post'><button>Configure Domoticz</button></form>"
#endif  // USE_DOMOTICZ
  "";
const char HTTP_BTN_MENU4[] PROGMEM =
  "<br/><form action='/lg' method='post'><button>Cấu hình log</button></form>"
  "<br/><form action='/co' method='post'><button>Configure Other</button></form>"
  "<br/><form action='/rt' method='post' onsubmit='return confirm(\"Xác nhận reset cấu hình\");'><button>Reset Cấu hình</button></form>"
  "<br/><form action='/dl' method='post'><button>Lưu cấu hình</button></form>"
  "<br/><form action='/rs' method='post'><button>Khôi phục cấu hình</button></form>";
const char HTTP_BTN_MAIN[] PROGMEM =
  "<br/><br/><form action='/' method='post'><button>Menu chính</button></form>";
const char HTTP_BTN_CONF[] PROGMEM =
  "<br/><br/><form action='/cn' method='post'><button>Menu chính</button></form>";
...
void stopWebserver()
{
  if (_httpflag) {
    webServer->close();
    _httpflag = HTTP_OFF;
    addLog_P(LOG_LEVEL_INFO, PSTR("HTTP: Webserver đã dừng"));
  }
}
...
void beginWifiManager()
{
  ...
  WiFi.softAP("STU_CONFIG","tunguyen"); //httu edited
  ...
  if (HTTP_MANAGER == _httpflag) {
    ...
    if (Maxdevice) {
      ...
      for (byte idx = 1; idx <= Maxdevice; idx++) {
        ...
        snprintf_P(line, sizeof(line), PSTR("<td style='width:%d%'><button onclick='la(\"?o=%d\");'>Bật/Tắt%s</button></td>"),
...
void handleUploadDone()
{
  ...
  if (_uploaderror) {
    page += F("<font color='red'>bị lỗi</font></b><br/><br/>");
    switch (_uploaderror) {
      case 1: strcpy_P(error, PSTR("Không có file")); break;
      case 2: strcpy_P(error, PSTR("Kích thước file lớn hơn không gian trống của thiết bị")); break;
      case 3: strcpy_P(error, PSTR("File magic header does not start with 0xE9")); break;
      case 4: strcpy_P(error, PSTR("Kích thước File lớn hơn kích thước flash của thiết bị")); break;
      case 5: strcpy_P(error, PSTR("File upload buffer miscompare")); break;
      case 6: strcpy_P(error, PSTR("Upload bị lỗi. Enable logging option 3 for more information")); break;
      case 7: strcpy_P(error, PSTR("Quá trình upload bị hủy")); break;
      case 8: strcpy_P(error, PSTR("Invalid configuration file")); break;
      case 9: strcpy_P(error, PSTR("File cấu hình quá lớn")); break;
      default:
