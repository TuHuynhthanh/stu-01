...
#define MODULE                 STU01 // [Module điều khiển 1 thiết bị] định nghĩa board sử dụng , khai báo trong file sonoff_template.h
...
void setup()
{
  ...
  if (strstr(sysCfg.hostname, "%")) {
    strlcpy(sysCfg.hostname, WIFI_HOSTNAME, sizeof(sysCfg.hostname)); //copy wifi hostname từ sysCfg.hostname vào WIFI_HOSTNAME
...
void loop()
{
  osw_loop(); //khởi động watchdog timer
